...
async function psxGet<T = unknown>(path: string): Promise<T> {
  ...
  if (!res.data.success)
    throw new Error(`PSX ${path}: ${JSON.stringify(res.data.error)}`);
...
async function fetchPsxKlines(symbol: string): Promise<OhlcvBar[]> {
  const startMs = Date.now() - 29 * 24 * 60 * 60 * 1000; // 29 days
  const data = await psxGet<Array<Record<string, number>>>(
...
async function fetchFundamentals(symbol: string): Promise<Fundamentals> {
  try {
    const d = await psxGet<Record<string, number | string>>(
    ...
    return {
      ...
      pbRatio:
        pe != null && typeof d.bookValue === "number" && d.bookValue > 0
          ? round2(pe / (d.bookValue as number))
          : null,
    };
  } catch {
    return {};
...
async function fetchDividends(symbol: string): Promise<DividendRecord[]> {
  try {
    const data = await psxGet<Array<Record<string, unknown>>>(
...
async function fetchYahooKlines(symbol: string): Promise<OhlcvBar[]> {
  ...
  const res = await yf.chart(symbol + ".KA", {
    period1,
    period2,
    interval: "1d",
  });
...
export async function fetchAllStocks(
  ...
  const [kse100, breadth] = await Promise.all([
    fetchKse100(),
    fetchMarketBreadth(),
  ]);
    ...
    try {
      ...
      console.log(
        `    ✓ ${symbol.padEnd(8)} PKR ${String(data.price).padStart(
          8
        )}${chg}  RSI:${data.rsi14}  MFI:${data.mfi}  ROC:${data.roc}%${st}  ${
          data.trend
        }`
      );
    } catch (err) {
