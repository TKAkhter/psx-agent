...
// ─────────────────────────────────────────────────────────────
//  SYSTEM INSTRUCTION  (sent once — saves tokens per call)
// ─────────────────────────────────────────────────────────────

const SYSTEM_INSTRUCTION = {
  parts: [
    {
      text: `You are a senior Pakistan Stock Exchange (PSX/KSE-100) expert with two roles:
1. Quant Analyst — validate algorithmic signals using macro + sector fundamentals
2. Retail Coach — explain trades in plain English with exact PKR amounts

PSX sector knowledge (always apply):
• Banking (MEBL): SBP rate cuts=wider spreads=bullish NIMs; watch KIBOR.
• Oil/Gas (OGDC/MARI/POL): Brent + domestic gas allocation = key drivers.
• Fertilizer (EFERT/FFC): Urea prices + SNGPL gas tariff = margins.
• Cement (LUCK): PSDP spending + construction credit + export margins.
• Energy/IPP (HUBC): Circular debt + capacity payments + tariff talks.
• Technology (SYS): IT exports + PKR/USD + US tech demand.
• Conglomerate (ENGROH): Diversified — fertilizer, energy, food.

On SELL signals: always suggest 1-2 better PSX stocks to buy instead.
...
//  Compact keys → expandMarket() translates for templates
// ─────────────────────────────────────────────────────────────
...
async function fetchMarketIntelligence(
  ...
  const live = liveMarket?.kse100
    ? `Live(PSXTerminal):KSE100=${liveMarket.kse100.level}(${liveMarket.kse100.changePct}%),Adv=${liveMarket.breadth?.advances},Dec=${liveMarket.breadth?.declines},A/D=${liveMarket.breadth?.adRatio}. Verify/fill via Search.`
    : "Use Search for all live figures.";

  const prompt = `Today:${today}. ${live}
Return JSON (null if unknown):
{"g":{"sent":"Risk-On|Risk-Off|Neutral","oil":"<Brent>","oilDir":"Rising|Falling|Stable","pkrusd":"<rate>","fed":"Hawkish|Dovish|Neutral|Pause","us10y":"<yield>","em":"Inflows|Outflows|Mixed","drivers":["<d1>","<d2>","<d3>"]},"pk":{"kse100":"<level>","kse100Chg":"<pct>","sbp":"<rate>","sbpOut":"Cut|Hold|Hike","cpi":"<pct>","cpiDir":"Falling|Stable|Rising","pkrOut":"Stable|Weak|Strong","imf":"OnTrack|AtRisk|Off","fx":"<bn USD>","polRisk":"Low|Med|High","risks":["<r1>","<r2>"],"winds":["<w1>","<w2>"]},"sectors":{"Banking":"<Bull|Bear|Neutral>|<reason>","Oil & Gas":"<Bull|Bear|Neutral>|<reason>","Fertilizer":"<Bull|Bear|Neutral>|<reason>","Cement":"<Bull|Bear|Neutral>|<reason>","Energy":"<Bull|Bear|Neutral>|<reason>","Technology":"<Bull|Bear|Neutral>|<reason>","Conglomerate":"<Bull|Bear|Neutral>|<reason>"},"generalMarketAnalysis":"<3 sentences: overall PSX market state today, breadth, key themes>","stance":"Bull|Bear|Neutral","headline":"<1 sentence: most important PSX driver today>"}`;
...
//  SELL signals include alt_buy_suggestions
// ─────────────────────────────────────────────────────────────
  ...
  const sigs = (Object.entries(signals) as [string, Signal][])
    ...
    .map(([, s]) => {
      ...
      return {
        ...
        mfi: ts.mfi,
        roc: ts.roc,
        st: ts.superTrend?.signal,
        regime: ts.marketRegime,
        trend: ts.trend,
        adx: ts.adx?.adx,
...
${perfLine}
Port:cost=PKR${summary.totalCost?.toLocaleString()} val=PKR${summary.totalValue?.toLocaleString()} pnl=${
    summary.totalPnlPct
  }%
...
Return JSON:
{"health":{"concRisk":"Low|Med|High","concDetail":"<overweight?>","pnlComment":"<1 line>","best":"<SYM—why>","risk":"<SYM—why>"},"v":[{"sym":"SYM","act":"BUY|SELL|HOLD|STRONG_BUY|STRONG_SELL","verdict":"Agree|Disagree|Partial","conv":"High|Med|Low","note":"<2 sentences: technicals+sector catalyst>","altAct":null,"altPx":null,"catalyst":"<named trigger>","risk":"<named risk>","horizon":"1-3d|1-2w|1-3m","simple":"<1-2 plain English with PKR>","entryZone":"<PKR range>","exitZone":"<PKR range>","alt_buy_suggestions":["<SYM: reason to buy instead — only for SELL signals, null otherwise>"]}],"macro":"<how macro affects this portfolio today>","generalMarketAnalysis":"<3 sentences: PSX market state, what sectors are hot/cold, what type of stocks to favour today>","topTrade":"<SYM—why best R/R>","avoid":"<SYM or null>","sectorRotation":"<any rotation opportunity? which sector moving money?>","tip":"<1 actionable tip>","mood":"Confident|Cautious|Patient|Defensive","stance":"Bull|Bear|Neutral"}`;
...
Use Google Search for upcoming catalysts (results, board meetings, dividends, PSX announcements).
Return JSON:
...
export async function getGeminiInsight(
  ...
  try {
    console.log("  → Phase 2: Validation + coaching...");
...
function expandAnalysisRaw(a: any): SignalAnalysis {
  ...
  return {
    ...
    validation: (a.v ?? []).map((v: any) => ({
      ...
      alt_buy_suggestions: v.alt_buy_suggestions ?? null,
    })),
