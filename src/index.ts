...
function buildWhatsAppSummary(signals: TradeSignalMap, summary: ReturnType<typeof calcPortfolioSummary>, timeStamp: string): string {
  ...
  for (const [sym, s] of Object.entries(signals)) {
    ...
    msg += `    RSI:${ts.rsi14}  MFI:${ts.mfi}  ST:${ts.superTrend?.signal ?? "—"}  ${ts.trend}\n\n`;
  }
...
function printSummary(signals: TradeSignalMap, summary: ReturnType<typeof calcPortfolioSummary>): void {
  head("SIGNAL SUMMARY");
  const ICONS: Record<string, string> = {
    STRONG_BUY: "🟢🟢", BUY: "🟢  ", HOLD: "⏸   ", SELL: "🔴  ", STRONG_SELL: "🔴🔴", SKIP: "⚫  ",
  };
  for (const [sym, s] of Object.entries(signals)) {
    if (s.action === "SKIP") continue;
    const ts   = s as TradeSignal;
    const icon = ICONS[ts.action] ?? "•  ";
    const chg  = ts.changePct != null ? ` (${ts.changePct >= 0 ? "+" : ""}${ts.changePct}%)` : "";
    const st   = ts.superTrend ? ` ST:${ts.superTrend.signal}` : "";
    console.log(`\n${icon} ${sym.padEnd(8)} PKR ${String(ts.price).padStart(8)}${chg}  [${ts.action}] ${ts.confidence}`);
    console.log(`         ${ts.instruction}`);
    if (ts.action !== "HOLD") {
      console.log(`         Target:${ts.targetPrice}  Stop:${ts.stopLoss}  R/R 1:${ts.rrRatio}`);
      console.log(`         RSI:${ts.rsi14}  MFI:${ts.mfi}  ROC:${ts.roc}%${st}  Regime:${ts.marketRegime}`);
    }
  }
  console.log(`\n${DASH}`);
...
async function main(): Promise<void> {
  ...
  head(`PSX Agent v6  ·  ${stampPKT()}  ·  Type:${ENV.PORTFOLIO_TYPE}  ·  Theme:${ENV.EMAIL_THEME}`);
  if (isWeekend()) { console.log("\n  🏖  Market closed (weekend)\n"); process.exit(0); }

  // ── 1. Database ───────────────────────────────────────────
  step(1, "Database");
  try { await db.connectDB(); }
  catch (err) { console.error(`  ✗ MongoDB: ${(err as Error).message}`); process.exit(1); }
  ...
  // ── 3. Market Data ────────────────────────────────────────
  step(3, `Market data  (${ENV.PORTFOLIO_TYPE === "psx" ? "PSXTerminal.com" : "Yahoo Finance"})`);
  ...
  } else { console.log("  ℹ  No previous session"); }

  // ── 6. Gemini ─────────────────────────────────────────────
  const isFirstSession = sessionHour < 11;
  step(6, `Gemini AI (Phase 1+2${isFirstSession ? "+3 weekly" : ""})`);
  const timeStamp = stampPKT();
  const gemini    = await getGeminiInsight(stockData, signals, summary, performance, timeStamp, sessionHour);
  console.log(gemini?.market   ? "  ✓ Phase 1 (market intel + Search)" : "  ⚠ Phase 1 failed");
  console.log(gemini?.analysis ? "  ✓ Phase 2 (validation + coaching)" : "  ⚠ Phase 2 failed");
  if (isFirstSession) console.log(gemini?.weekly ? "  ✓ Phase 3 (weekly review)" : "  ⚠ Phase 3 failed");

  // ── 7. Generate PDF & Send ────────────────────────────────
  step(7, "Generating PDF & sending notifications");
  const reportData: ReportData = { stockData, signals, summary, performance, gemini, timeStamp, sessionHour };
  const pdfPath = getPdfPath(timeStamp);

  try {
    console.log(`  Generating PDF → ${pdfPath}`);
    await generatePdfReport(reportData, pdfPath);
    console.log(`  ✓ PDF generated (${Math.round(require("fs").statSync(pdfPath).size / 1024)}KB)`);
  } catch (err) {
  ...
  if (emailRes.status   === "rejected") console.error(`  ✗ Email:      ${(emailRes.reason as Error).message}`);
  if (waTextRes.status  === "rejected") console.error(`  ✗ WA text:   ${(waTextRes.reason as Error).message}`);
  if (waPdfRes.status   === "rejected") console.error(`  ✗ WA PDF:    ${(waPdfRes.reason as Error).message}`);

  // Save session
  try {
    await saveSession(signals, summary, stockData, gemini?.analysis?.overall_stance ?? null);
    console.log("  ✓ Session saved to MongoDB");
  } catch (err) { console.warn(`  ⚠ Save: ${(err as Error).message}`); }

  printSummary(signals, summary);
  console.log(`\n  ✓ Done in ${((Date.now() - startMs) / 1000).toFixed(1)}s  ·  ${stampPKT()}`);
