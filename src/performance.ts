...
import type { TradeSignalMap, PortfolioSummary, PerformanceResult, BreakdownEntry, StockDataMap, StockData } from "./types";
...
export async function evaluatePerformance(currentData: StockDataMap): Promise<PerformanceResult | null> {
  try {
    const docs = await db.findMany<SavedSession>("signals", {}, { sort: { createdAt: -1 }, limit: 1 } as Parameters<typeof db.findMany>[2]);
    ...
    for (const [ticker, entry] of Object.entries(currentData)) {
      ...
      const ok     = isBuy ? curr.price >= prevSnap.price : isSell ? curr.price <= prevSnap.price : Math.abs(delta) < 2;
      if (ok) correct++;
...
export async function saveSession(
  ...
  for (const [sym, s] of Object.entries(signals)) signalMap[sym] = { action: s.action };
  await db.insertOne("signals", { createdAt: new Date(), signals: signalMap, summary, geminiStance, snapshot });
}