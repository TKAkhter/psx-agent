...
export async function loadPortfolio(): Promise<PortfolioEntry[]> {
  ...
  if (count === 0) {
    console.log(`  ℹ  Portfolio empty (type=${PORTFOLIO_TYPE}) — seeding defaults...`);
  ...
  console.log(`  ✓  ${positions.length} positions loaded (type=${PORTFOLIO_TYPE})`);
  return positions;
}
...
export function buildPortfolioMap(positions: PortfolioEntry[]): PortfolioMap {
  ...
  for (const p of positions) {
    map[p.ticker] = { symbol: p.symbol, name: p.name, sector: p.sector, shares: p.shares, avgCost: p.avgCost };
  }
  return map;
}