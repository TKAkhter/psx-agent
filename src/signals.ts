...
function buildSignal(symbol: string, d: StockData): TradeSignal {
  ...
  if (d.mfi != null) {
    if (d.mfi < 20) {
      ...
      bullReasons.push(`MFI ${d.mfi} — oversold (volume-confirmed)`);
    } else if (d.mfi < 30) {
      score += SCORE_WEIGHTS.MFI_NORMAL;
      bullReasons.push(`MFI ${d.mfi} — approaching oversold`);
    } else if (d.mfi > 80) {
      score -= SCORE_WEIGHTS.MFI_EXTREME;
      bearReasons.push(`MFI ${d.mfi} — overbought (volume-confirmed)`);
    } else if (d.mfi > 70) {
      score -= SCORE_WEIGHTS.MFI_NORMAL;
  ...
  if (d.roc != null) {
    if (d.roc < -15) {
      ...
      bullReasons.push(
        `ROC ${d.roc}% — deep negative momentum, mean-reversion setup`
      );
    } else if (d.roc < -8) {
      score += SCORE_WEIGHTS.ROC_MILD;
      bullReasons.push(`ROC ${d.roc}% — negative momentum, potential bounce`);
    } else if (d.roc > 15) {
      score -= SCORE_WEIGHTS.ROC_STRONG;
      bearReasons.push(`ROC ${d.roc}% — overextended momentum`);
    } else if (d.roc > 8) {
      score -= SCORE_WEIGHTS.ROC_MILD;
      bearReasons.push(`ROC ${d.roc}% — watch for momentum exhaustion`);
    }
  }
  ...
  if (d.superTrend) {
    if (d.superTrend.signal === "BUY") {
      ...
      bullReasons.push(
        `SuperTrend BUY @ PKR ${d.superTrend.value} (${d.superTrend.distance}% above ST line)`
      );
    } else {
      score -= SCORE_WEIGHTS.SUPERTREND;
      bearReasons.push(
        `SuperTrend SELL @ PKR ${d.superTrend.value} (${Math.abs(
  ...
  if (d.stoch?.k != null) {
    ...
    if (k < 15 && (dL ?? 100) < 15) {
      ...
      bullReasons.push(`Stoch %K ${k}/%D ${dL} — extreme oversold`);
    } else if (k < 20 && (dL ?? 100) < 20) {
      score += SCORE_WEIGHTS.STOCH_NORMAL;
      bullReasons.push(`Stoch %K ${k}/%D ${dL} — oversold`);
    } else if (k > 85 && (dL ?? 0) > 85) {
      score -= SCORE_WEIGHTS.STOCH_EXTREME;
      bearReasons.push(`Stoch %K ${k}/%D ${dL} — extreme overbought`);
    } else if (k > 80 && (dL ?? 0) > 80) {
      score -= SCORE_WEIGHTS.STOCH_NORMAL;
      bearReasons.push(`Stoch %K ${k}/%D ${dL} — overbought`);
    }
  ...
  if (d.macd?.macd != null) {
    if (d.macd.crossover === "BULLISH_CROSS") {
      ...
      d.macd.histogram != null &&
      d.macd.histogram > 0 &&
      d.macd.histTrend === "EXPANDING"
    ) {
      score += SCORE_WEIGHTS.MACD_HISTOGRAM;
      bullReasons.push(`MACD histogram expanding bullish`);
    } else if (
      d.macd.histogram != null &&
      d.macd.histogram < 0 &&
      d.macd.histTrend === "EXPANDING"
      ...
      score -= SCORE_WEIGHTS.MACD_HISTOGRAM;
      bearReasons.push(`MACD histogram expanding bearish`);
    } else if (d.macd.histogram != null && d.macd.histogram > 0) {
  ...
  if (d.bb?.pctB != null) {
    ...
    if (pctB < 0) {
      ...
      bullReasons.push(`BB %B ${round2(pctB)} — below lower band`);
    } else if (pctB < 10) {
      ...
      score -= SCORE_WEIGHTS.BB_EXTREME;
      bearReasons.push(`BB %B ${round2(pctB)} — above upper band`);
    } else if (pctB > 90) {
      score -= SCORE_WEIGHTS.BB_NEAR + 1;
    ...
    if (squeeze)
      neutralNotes.push(
        `BB squeeze (${d.bb.bandwidth}%) — breakout approaching`
      );
  }

  // ── Market Regime ─────────────────────────────────────────
  if (d.marketRegime === "TRENDING_BULL") {
    score += SCORE_WEIGHTS.MARKET_REGIME;
    bullReasons.push(`Market regime: TRENDING BULL — trend signals reliable`);
  } else if (d.marketRegime === "TRENDING_BEAR") {
    score -= SCORE_WEIGHTS.MARKET_REGIME;
    bearReasons.push(`Market regime: TRENDING BEAR`);
  } else if (d.marketRegime === "BREAKOUT") {
    score += SCORE_WEIGHTS.MARKET_REGIME;
    bullReasons.push(`Market regime: BREAKOUT from squeeze`);
  } else if (d.marketRegime === "BREAKDOWN") {
    score -= SCORE_WEIGHTS.MARKET_REGIME;
    bearReasons.push(`Market regime: BREAKDOWN from squeeze`);
  } else
    neutralNotes.push(
      `Market regime: RANGING — oscillators more reliable than trend signals`
    );
  ...
  if (d.adx?.adx != null) {
    ...
    if (
      ...
      (strength === "VERY_STRONG" && (diPlus ?? 0) > (diMinus ?? 0))
    ) {
      ...
      score -= SCORE_WEIGHTS.ADX_STRONG;
      bearReasons.push(`ADX ${adxVal} — strong bear`);
  ...
  if (d.ma20 != null) {
    d.price > d.ma20
      ? ((score += SCORE_WEIGHTS.MA_CROSS),
        bullReasons.push(`Price above MA20 ${d.ma20}`))
      : ((score -= SCORE_WEIGHTS.MA_CROSS),
        bearReasons.push(`Price below MA20 ${d.ma20}`));
  }
  if (d.ma5 != null && d.ma20 != null) {
    d.ma5 > d.ma20
      ? ((score += SCORE_WEIGHTS.MA_CROSS),
        bullReasons.push(`MA5 > MA20 — short bull`))
      : ((score -= SCORE_WEIGHTS.MA_CROSS),
        bearReasons.push(`MA5 < MA20 — short bear`));
  }
  ...
  if (d.ma200 != null) {
    d.price > d.ma200
      ? ((score += SCORE_WEIGHTS.MA_CROSS),
        bullReasons.push(`Price above MA200 — long-term bull`))
      : ((score -= SCORE_WEIGHTS.MA_CROSS),
        bearReasons.push(`Price below MA200 — long-term bear`));
  }
  if (d.ema9 != null && d.ema21 != null) {
    d.ema9 > d.ema21
      ? ((score += SCORE_WEIGHTS.MA_CROSS),
        bullReasons.push(`EMA9 > EMA21 — momentum up`))
      : ((score -= SCORE_WEIGHTS.MA_CROSS),
        bearReasons.push(`EMA9 < EMA21 — momentum down`));
  }

  // ── VWAP ─────────────────────────────────────────────────
  if (d.vwapDevPct != null) {
    if (d.vwapDevPct < -5) {
      score += SCORE_WEIGHTS.VWAP + SCORE_WEIGHTS.VWAP_DEV;
      bullReasons.push(
        `Price ${d.vwapDevPct}% below VWAP — deeply undervalued vs fair value`
      );
    } else if (d.vwapDevPct < -2) {
      score += SCORE_WEIGHTS.VWAP;
      bullReasons.push(`Price ${d.vwapDevPct}% below VWAP`);
    } else if (d.vwapDevPct < 0) {
      score += 1;
      bullReasons.push(`Price slightly below VWAP`);
    } else if (d.vwapDevPct > 5) {
      score -= SCORE_WEIGHTS.VWAP + SCORE_WEIGHTS.VWAP_DEV;
      bearReasons.push(`Price ${d.vwapDevPct}% above VWAP — stretched`);
    } else if (d.vwapDevPct > 2) {
      score -= SCORE_WEIGHTS.VWAP;
      bearReasons.push(`Price ${d.vwapDevPct}% above VWAP`);
    } else if (d.vwapDevPct > 0) {
      score -= 1;
      bearReasons.push(`Price slightly above VWAP`);
    }
  }
  ...
  if (d.ichi) {
    ...
    if (d.ichi.tkBullish) {
      ...
      score -= SCORE_WEIGHTS.ICHI_TK;
      bearReasons.push(`TK: Tenkan < Kijun`);
    }
    if (d.ichi.chikouBullish != null) {
      d.ichi.chikouBullish
        ? bullReasons.push("Chikou bullish")
        : bearReasons.push("Chikou bearish");
    }
  }
  ...
  if (d.vol.volSpike) {
    if (score > 0) {
      ...
      bullReasons.push(`Volume ${d.vol.volRatio}x avg — confirms bullish`);
    } else {
      score -= SCORE_WEIGHTS.VOLUME_CONFIRM;
      bearReasons.push(`Volume ${d.vol.volRatio}x avg — confirms bearish`);
    }
  }
  ...
  if (d.pivots) {
    ...
    if (d.price <= s1 * 1.005 && d.price > s2) {
      ...
      bullReasons.push(`Near S1 support ${s1}`);
    }
    if (d.price <= s2 * 1.005) {
      score += SCORE_WEIGHTS.PIVOT_LEVEL;
    ...
    if (d.price >= r1 * 0.995 && d.price < r2) {
      score -= SCORE_WEIGHTS.PIVOT_LEVEL;
      bearReasons.push(`Near R1 resistance ${r1}`);
    }
    if (d.price >= r2 * 0.995) {
      score -= SCORE_WEIGHTS.PIVOT_LEVEL;
  ...
  if (pe != null) {
    if (pe < 6) {
      ...
      bullReasons.push(`P/E ${pe}x — very cheap`);
    } else if (pe > 30) {
      score -= SCORE_WEIGHTS.FUNDAMENTAL_PE;
      bearReasons.push(`P/E ${pe}x — expensive`);
    }
  }
  if (dy != null && dy > 8) {
    score += SCORE_WEIGHTS.FUNDAMENTAL_DIV;
    bullReasons.push(`Dividend yield ${dy}% — attractive`);
  }
  if (pb != null) {
    if (pb < 1) {
      score += SCORE_WEIGHTS.FUNDAMENTAL_PB;
      bullReasons.push(`P/B ${pb}x — below book value`);
    } else if (pb > 4) {
      score -= SCORE_WEIGHTS.FUNDAMENTAL_PB;
      bearReasons.push(`P/B ${pb}x — pricey vs book`);
    }
  }

  // ── P&L context ───────────────────────────────────────────
  if ((d.unrealizedPct ?? 0) < -25)
    neutralNotes.push(
      `⚠️ Down ${Math.abs(d.unrealizedPct!)}% from cost — re-evaluate thesis`
    );
  ...
  const proSummary = `Score ${score >= 0 ? "+" : ""}${score} | ${
    d.trend
  } | Regime:${d.marketRegime} | RSI:${d.rsi14} MFI:${d.mfi} ROC:${d.roc}% ST:${
    d.superTrend?.signal ?? "—"
  } | MACD:${d.macd?.crossover ?? d.macd?.histTrend} | BB%B:${
    d.bb?.pctB
  } | ADX:${d.adx?.adx}(${d.adx?.strength}) | Ichi:${
    d.ichi?.position ?? "N/A"
  } | VWAP±${d.vwapDevPct}%`;
  ...
  for (const [symbol, entry] of Object.entries(stockData)) {
    ...
    if (!d.price || isStockError(d)) {
      signals[symbol] = {
        symbol,
        action: "SKIP",
        error: isStockError(d) ? d.error : "No price",
        score: 0,
        price: null,
      };
    } else {
...
export function calcPortfolioSummary(
  ...
  for (const [key, entry] of Object.entries(stockData)) {
    ...
    if (sd.sector)
      sectorMap[sd.sector] = (sectorMap[sd.sector] ?? 0) + sd.marketValue;
  }
